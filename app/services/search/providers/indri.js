...
if (index_path) {
    ...
    try {
        ...
        if (e instanceof Error && e.code === "MODULE_NOT_FOUND") {
            console.error(
                "Node-indri was not found, please install it if you wish to use the indri provider. See the documentation for instructions."
            );
        } else {
            console.error(e);
        }
...
exports.fetch = function (
    ...
    return new Promise(function (resolve, reject) {
        ...
        query = escape(query);
        relevanceFeedbackDocuments = relevanceFeedbackDocuments.map((string) =>
            parseInt(string)
        );
        try {
            indri_searcher.search(
                query,
                pageNumber,
                resultsPerPage,
                relevanceFeedbackDocuments,
